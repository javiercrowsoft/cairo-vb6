  ...
  declare NOMBRE_PRESTACION int set NOMBRE_PRESTACION = ID_PRESTACION
	...
	if PARAM_ID <> 0 begin
	  ...
	  from NOMBRE_TABLA where CAMPO_ID = PARAM_ID
		...
		-- Tiene permiso para editar NOMBRE_DOCUMENTO
		--
		...
		select @pre_id_edit = pre_id_edit, @doc_nombre = doc_nombre from documento where doc_id = @doc_id
		if not exists (select per_id from permiso 
	                   where pre_id = @pre_id_edit
	                         and (
																	(
																	us_id = @@us_id
																	)
						...
						set @@bEditable = 0
	          set @@editMsg = 'Usted no tiene permiso para editar ' + @doc_nombre
						if @@ShowMsg <> 0
							select [Editable]=@@bEditable, [EditMsg]= @@editMsg
	          return
		end
