if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_web_deudaCancelada]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_web_deudaCancelada]
...
go
create procedure sp_web_deudaCancelada (
  @@proveedor     int,
  @@empresa       int,
  @@concepto       int
)
