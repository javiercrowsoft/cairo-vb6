...
Public Sub PrinterSetSizeAndOrient(PrinterName As String, _
  ...
  If OpenPrinter(PrinterName, hPrn, 0&) Then
    ...
    If LastError <> 0 Then
      Err.Raise vbObjectError + 15, "Ha ocurrido un error al setear la p�gina en la impresora. N�mero de error: " & LastError, vbExclamation, App.EXEName
    End If
...
Public Function PrinterSetPaperBin(PrinterName As String, _
  ...
  If OpenPrinter(PrinterName, hPrn, 0&) Then
    ...
    If LastError <> 0 Then
      Err.Raise vbObjectError + 15, "Ha ocurrido un error al setear la bandeja en la impresora. N�mero de error: " & LastError, vbExclamation, App.EXEName
    End If
...
Public Function GetPaperType(ByVal hwnd As Long, ByVal PrinterName As String) As Long
  ...
  Else
    LastError = GetLastError()
    Err.Raise vbObjectError + 15, "Ha ocurrido un error al obtener el tama�o del papel. N�mero de error: " & LastError, vbExclamation, App.EXEName
  End If
End Function
