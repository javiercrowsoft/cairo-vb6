if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocImportacionTempEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocImportacionTempEditableGet]

go
/*

sp_DocImportacionTempEditableGet 57,7,0,'',1

*/

create procedure sp_DocImportacionTempEditableGet (
	@@emp_id    		int,
	@@impt_id 			int,
  @@us_id     		int,
...
begin
  ...
  declare @csPreExpEditImportacionTmp 	int set @csPreExpEditImportacionTmp 	= 22032
	declare @csPreExpDeleteImportacionTmp	int set @csPreExpDeleteImportacionTmp	= 22033
	...
	if @@impt_id <> 0 begin
	  ...
	  from ImportacionTemp c inner join Documento d on c.doc_id = d.doc_id 
...
--TODO:delete
		declare @pre_id int
		if @@bDelete = 0	set @pre_id = @csPreExpEditImportacionTmp
		else							set @pre_id = @csPreExpDeleteImportacionTmp
--TODO:delete

		-- Tiene permiso para editar importaciones temporales
		--
...
--TODO:delete
						if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar importaciones temporales'
						else							set @@editMsg = 'Usted no tiene permiso para borrar importaciones temporales'
--TODO:delete
	  ...
	  select @fca_id = fca_id from Documento where doc_id = @doc_id 
		...
		if not @fca_id is null begin
		  ...
		  if not exists(select fca_id from FechaControlAcceso 
		            where fca_id = @fca_id and @impt_fecha between fca_fechaDesde and fca_fechaHasta) begin
