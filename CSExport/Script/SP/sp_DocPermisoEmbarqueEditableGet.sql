if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocPermisoEmbarqueEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocPermisoEmbarqueEditableGet]

go
/*

sp_DocPermisoEmbarqueEditableGet 21,6,7,0,'',1

*/

create procedure sp_DocPermisoEmbarqueEditableGet (
	@@emp_id    		int,
	@@pemb_id 			int,
  @@us_id     		int,
...
begin
  ...
  declare @csPreExpEditPermisoEmbarque 		int set @csPreExpEditPermisoEmbarque 		= 22010
	declare @csPreExpDeletePermisoEmbarque	int set @csPreExpDeletePermisoEmbarque	= 22011
	...
	if @@pemb_id <> 0 begin
	  ...
	  from PermisoEmbarque c inner join Documento d on c.doc_id = d.doc_id 
...
--TODO:delete
		declare @pre_id int
		if @@bDelete = 0	set @pre_id = @csPreExpEditPermisoEmbarque
		else							set @pre_id = @csPreExpDeletePermisoEmbarque
--TODO:delete

		-- Tiene permiso para editar permisos de embarque
		--
...
--TODO:delete
						if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar permisos de embarque'
						else							set @@editMsg = 'Usted no tiene permiso para borrar permisos de embarque'
--TODO:delete
	  ...
	  select @fca_id = fca_id from Documento where doc_id = @doc_id 
		...
		if not @fca_id is null begin
		  ...
		  if not exists(select fca_id from FechaControlAcceso 
		            where fca_id = @fca_id and @pemb_fecha between fca_fechaDesde and fca_fechaHasta) begin
