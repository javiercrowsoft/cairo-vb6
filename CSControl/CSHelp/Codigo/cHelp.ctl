...
Private Const c_SPFilter = "SPFilter"
Private Const c_SPInfoFilter = "SPInfoFilter"
...
Private Const c_KeyHelp = "KeyHelp"
Private Const c_KeyInfo = "KeyInfo"
...
Public Property Get KeyHelp() As Long
  KeyHelp = m_KeyHelp
End Property

Public Property Let KeyHelp(ByVal rhs As Long)
  m_KeyHelp = rhs
  PropertyChanged c_KeyHelp
End Property

Public Property Get KeyInfo() As Long
  KeyInfo = m_KeyInfo
End Property

Public Property Let KeyInfo(ByVal rhs As Long)
  m_KeyInfo = rhs
  PropertyChanged c_KeyInfo
End Property
...
Public Property Let Table(ByVal rhs As csTables)
  ...
  m_Table = rhs
  Dim Hlp As CSOAPI2.cHelp
  Set Hlp = New CSOAPI2.cHelp
  
  m_HaveInfo = Hlp.HaveInfo(m_Table)

  GoTo ExitProc
ControlError:
  MngError Err, c_Table, C_Module, vbNullString
...
Public Property Let Filter(ByVal rhs As String)
  m_Filter = rhs
End Property
Public Property Get SPFilter() As String
  SPFilter = m_SPFilter
End Property
Public Property Let SPFilter(ByVal rhs As String)
  m_SPFilter = rhs
End Property
Public Property Get SPInfoFilter() As String
  SPInfoFilter = m_SPInfoFilter
End Property
Public Property Let SPInfoFilter(ByVal rhs As String)
  m_SPInfoFilter = rhs
End Property
...
Public Property Let NoSel(ByVal rhs As Boolean)
  m_NoSel = True
End Property
' propiedades privadas
' funciones publicas
Public Sub RaiseEventChange()
  On Error Resume Next
  RaiseEvent Change
End Sub
...
Private Sub imgInfo_Click()
  ...
  With Extender
    ...
    ' No se por que pero sin esto no anda
    '  .Left + (TxValue.Width * Screen.TwipsPerPixelX)
    '
    c.ShowInfoEx m_Table, Val(m_Id), m_SPInfoFilter, _
                 UserControl.hWnd, _
                 .Left + (TxValue.Width * Screen.TwipsPerPixelX), _
                 .Top, _
                 Me
  End With
  ...
  GoTo ExitProc
ControlError:
  MngError Err, "imgInfo_Click", C_Module, vbNullString
  If Err.Number Then Resume ExitProc
ExitProc:
  On Error Resume Next
End Sub
...
Private Sub PicButton_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
  ...
  If Button = vbRightButton Then
    ...
    With Extender
      ...
      '  .Left + (TxValue.Width * Screen.TwipsPerPixelX)
      '
      c.ShowMenuEx m_Table, Val(m_Id), m_SPInfoFilter, _
                   UserControl.hWnd, _
                   .Left + (TxValue.Width * Screen.TwipsPerPixelX), _
                   .Top, _
                   Me
...
Private Sub TxValue_KeyDown(KeyCode As Integer, Shift As Integer)
  On Error Resume Next
  If KeyCode = m_KeyHelp And Shift = 0 Then ShowHelp
  If KeyCode = m_KeyInfo And Shift = 0 Then pShowInfo
...
Private Sub UserControl_Resize()
  ...
  ButtonHeight = UserControl.ScaleHeight
  ...
  With picInfo
    If Val(m_Id) <> csNO_ID And m_HaveInfo Then
      CreateImage imgInfo, 101
      ButtonWidth2 = picInfo.Width
      .Move UserControl.ScaleWidth - ButtonWidth - ButtonWidth2, _
            InternalFrame, _
            .Width, _
            ButtonHeight - (InternalFrame * 2)
      .Visible = True
    Else
      .Visible = False
      ButtonWidth2 = 0
  ...
  With TxValue
    If Left$(ValueHelp, 1) = KEY_NODO Then
      With picFolder
        CreateImage imgFolder, 102
        .Left = InternalFrame
        .Height = ButtonHeight - (InternalFrame * 2)
        .Top = InternalFrame
        .Visible = True
        offset = .Width
      End With
      .Move offset, _
            InternalFrame, _
            ScaleWidth - ButtonWidth - offset - ButtonWidth2, _
            ButtonHeight - (InternalFrame * 2)
    Else
      picFolder.Visible = False
      .Move InternalFrame, _
            InternalFrame, _
            ScaleWidth - ButtonWidth - InternalFrame - ButtonWidth2, _
            ButtonHeight - (InternalFrame * 2)
    End If
  End With
  DrawSelectionBox UNPRESSED
...
Public Sub Validate()
  ...
  Help.ForAbm = m_bForAbm
  Set hr = Help.ValidateEx(m_Table, TxValue.Text, ValueHelp, m_Filter, m_SPFilter)
...
Private Sub DrawSelectionBox(ByVal bStatus As STATUS_BUTTON)
  ...
  Select Case bStatus
    ...
    Case UNPRESSED
      If m_Enabled Then
        clrTop = m_BorderColor
        clrLeft = m_BorderColor
        clrBottom = m_BorderColor
  ...
  DrawObjBox PicButton.hDC, 0, 0, PicButton.ScaleWidth, PicButton.ScaleHeight, False, _
               0, 0, 1, clrLeft, clrBottom, csEBS3d, True, clrRight
...
Private Sub UserControl_InitProperties()
  ...
  m_Filter = vbNullString
  m_SPFilter = vbNullString
  m_SPInfoFilter = vbNullString
  m_HelpType = csNormal
  m_ColumnValueProcess = vbNullString
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
  ...
  With PropBag
    ...
    .WriteProperty c_ToolTipText, m_ToolTipText, vbNullString
    .WriteProperty c_Table, m_Table, csPrestacion
    .WriteProperty c_Filter, m_Filter, vbNullString
    .WriteProperty c_SPFilter, m_SPFilter, vbNullString
    .WriteProperty c_SPInfoFilter, m_SPInfoFilter, vbNullString
    .WriteProperty c_HelpType, m_HelpType, csNormal
    .WriteProperty c_ColumnValueProcess, m_ColumnValueProcess, vbNullString
    ...
    .WriteProperty c_FontName, TxValue.FontName, vbNullString
    ...
    .WriteProperty c_KeyHelp, m_KeyHelp, vbKeyF4
    .WriteProperty c_KeyInfo, m_KeyInfo, vbKeyF2
  End With
End Sub
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
  ...
  With PropBag
    ...
    ToolTipText = .ReadProperty(c_ToolTipText, vbNullString)
    Table = .ReadProperty(c_Table, csPrestacion)
    Filter = .ReadProperty(c_Filter, vbNullString)
    SPFilter = .ReadProperty(c_SPFilter, vbNullString)
    SPInfoFilter = .ReadProperty(c_SPInfoFilter, vbNullString)
    HelpType = .ReadProperty(c_HelpType, csNormal)
    ColumnValueProcess = .ReadProperty(c_ColumnValueProcess, vbNullString)
    ...
    KeyHelp = .ReadProperty(c_KeyHelp, vbKeyF4)
    KeyInfo = .ReadProperty(c_KeyInfo, vbKeyF2)
  End With
End Sub
