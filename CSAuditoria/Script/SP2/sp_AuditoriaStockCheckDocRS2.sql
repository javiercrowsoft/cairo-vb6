...
if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_AuditoriaStockCheckDocRS2]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_AuditoriaStockCheckDocRS2]

go

create procedure sp_AuditoriaStockCheckDocRS2 (
    ...
    --
      declare c_rs_item insensitive cursor for
        ...
        where   rs_id = @@rs_id and (pr_llevanroserie = 0 or pr_eskit <> 0)
            and  ((@@bNegativo = 0 and rsi_ajuste > 0) or (@@bNegativo <> 0 and rsi_ajuste < 0))
