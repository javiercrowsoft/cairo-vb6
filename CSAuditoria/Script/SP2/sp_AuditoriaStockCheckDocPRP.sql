...
if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_AuditoriaStockCheckDocPRP]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_AuditoriaStockCheckDocPRP]

go

create procedure sp_AuditoriaStockCheckDocPRP (
...
begin
  ...
  if @llevaStock <> 0 begin
    ...
    if @est_id = 7 begin
      ...
      if @st_id is not null begin
        ...
        if exists (select * from Stock where st_id = @st_id) begin
          ...
          set @bError = 1
          set @@bErrorMsg = @@bErrorMsg + 'El parte de reparación esta anulado y posee un movimiento de stock' + char(10)
    
        end else begin
                
          set @bError = 1
          set @@bErrorMsg = @@bErrorMsg + 'El parte de reparación esta anulado y posee st_id distinto de null pero este st_id no existe en la tabla stock' + char(10)
    ...
    -- 
      ...
      -- Sin numero de serie
      --
        declare c_prp_item insensitive cursor for
          ...
          select 
                  ...
                  stl_id
          from
                ParteReparacionItem prpi inner join Producto pr on prpi.pr_id = pr.pr_id
      
          where prp_id = @@prp_id and (pr_llevanroserie = 0 or pr_eskit <> 0)
        ...
        fetch next from c_prp_item into 
                                        @prpi_cantidad,
                                        @pr_id,
                                        @pr_nombreventa,
                                        @pr_llevastock,
        ...
        begin
          ...
          if @pr_llevastock <> 0 begin
            ...
            if @pr_eskit <> 0 begin
              set @cant_kits      = @prpi_cantidad
              set @prpi_cantidad  = @prpi_cantidad * @pr_kitItems
              ...
              set  @prpi_cantidad = @prpi_cantidad * @pr_ventastock
            ...
            if abs(@sti_cantidad - @prpi_cantidad) > 0.01 begin
  
              if @pr_eskit <> 0 begin
  
                set @bError = 1
                set @@bErrorMsg = @@bErrorMsg 
                                  + 'El parte de reparación indica ' + convert(varchar,convert(decimal(18,2),@cant_kits)) 
                                  + ' Kit "' + @pr_nombreventa + '" compuesto(s) en total por '
                                  + convert(varchar,convert(decimal(18,2),@prpi_cantidad)) + ' items'
                ...
                set @bError = 1
                set @@bErrorMsg = @@bErrorMsg 
                                  + 'El parte de reparación indica ' + convert(varchar,convert(decimal(18,2),@prpi_cantidad))
            ...
            if @pr_llevanroserie <> 0 and @pr_eskit <> 0 begin
              ...
              declare c_prp_itemKit insensitive cursor for
              ...
              fetch next from c_prp_itemKit into @pr_id_item, @pr_item, @prns_cantidad
              ...
              begin
                ...
                if @sti_cantidad <> @prns_cantidad begin
                  ...
                  set @bError = 1
                  set @@bErrorMsg = @@bErrorMsg 
                                    + 'El parte de reparación indica que el Kit "' + @pr_nombreventa +
            ...
            if exists(select * from StockItem where st_id = @st_id and pr_id = @pr_id) begin
              ...
              set @bError = 1
              set @@bErrorMsg = @@bErrorMsg 
                                + 'Este parte de reparación indica el producto "' + @pr_nombreventa 
                                + '" que no mueve stock pero esta incluido en el movimiento '
                                + 'de stock asociado a este parte de reparación '
                                + char(10)
      ...
      --
        declare c_prp_item insensitive cursor for
          ...
          where prp_id = @@prp_id and pr_llevanroserie <> 0 and pr_eskit = 0
        ...
        begin
          ...
          where 
            ...
            and sti_grupo        = @prpi_id
            and pr_id_kit is null
  
          set @sti_cantidad = IsNull(@sti_cantidad,0)
  
          if abs(@sti_cantidad - (@prpi_cantidad * @pr_ventastock)) > 0.01 begin
