if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocMovimientoFondoEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocMovimientoFondoEditableGet]

go
/*

sp_DocMovimientoFondoEditableGet 57,7,0,'',1

*/

create procedure sp_DocMovimientoFondoEditableGet (
	@@emp_id    		int,
	@@mf_id 				int,
  @@us_id     		int,
...
begin
  ...
  declare @csPreTsrEditMovimientoFondo 		int set @csPreTsrEditMovimientoFondo 		= 18021
	declare @csPreTsrDeleteMovimientoFondo	int set @csPreTsrDeleteMovimientoFondo	= 18022
	...
	if @@mf_id <> 0 begin
	  ...
	  from MovimientoFondo c inner join Documento d on c.doc_id = d.doc_id 
		where mf_id = @@mf_id
...
--TODO:delete
		declare @pre_id int
		if @@bDelete = 0	set @pre_id = @csPreTsrEditMovimientoFondo
		else							set @pre_id = @csPreTsrDeleteMovimientoFondo
--TODO:delete
...
--TODO:delete
						if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar movimientos de fondo'
						else							set @@editMsg = 'Usted no tiene permiso para borrar movimientos de fondo'
--TODO:delete
	  ...
	  select @fca_id = fca_id from Documento where doc_id = @doc_id 
		...
		if not @fca_id is null begin
		  ...
		  if not exists(select fca_id from FechaControlAcceso 
		            where fca_id = @fca_id and @mf_fecha between fca_fechaDesde and fca_fechaHasta) begin
