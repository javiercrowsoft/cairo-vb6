if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocDepositoCuponEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocDepositoCuponEditableGet]

go
/*

sp_DocDepositoCuponEditableGet 57,7,0,'',1

*/

create procedure sp_DocDepositoCuponEditableGet (
	@@emp_id      	int,
	@@dcup_id 			int,
  @@us_id     		int,
...
begin
  ...
  declare @csPreTsrEditDepositoCupon 		int set @csPreTsrEditDepositoCupon 		= 18039
	declare @csPreTsrDeleteDepositoCupon	int set @csPreTsrDeleteDepositoCupon	= 18040
	...
	if @@dcup_id <> 0 begin
	  ...
	  from DepositoCupon c inner join Documento d on c.doc_id = d.doc_id 
...
--TODO:delete
		declare @pre_id int
		if @@bDelete = 0	set @pre_id = @csPreTsrEditDepositoCupon
		else							set @pre_id = @csPreTsrDeleteDepositoCupon
--TODO:delete

		-- Tiene permiso para editar depositos de cupon
		--
...
--TODO:delete
						if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar depositos de cupon'
						else							set @@editMsg = 'Usted no tiene permiso para borrar depositos de cupon'
--TODO:delete
	  ...
	  select @fca_id = fca_id from Documento where doc_id = @doc_id 
		...
		if not @fca_id is null begin
		  ...
		  if not exists(select fca_id from FechaControlAcceso 
		            where fca_id = @fca_id and @dcup_fecha between fca_fechaDesde and fca_fechaHasta) begin
