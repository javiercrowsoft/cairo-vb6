if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocDepositoBancoEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocDepositoBancoEditableGet]

go
/*

sp_DocDepositoBancoEditableGet 57,7,0,'',1

*/

create procedure sp_DocDepositoBancoEditableGet (
	@@emp_id      	int,
	@@dbco_id 			int,
  @@us_id     		int,
...
begin
  ...
  declare @csPreTsrEditDepositoBanco 		int set @csPreTsrEditDepositoBanco 		= 18033
	declare @csPreTsrDeleteDepositoBanco	int set @csPreTsrDeleteDepositoBanco	= 18034
	...
	if @@dbco_id <> 0 begin
	  ...
	  from DepositoBanco c inner join Documento d on c.doc_id = d.doc_id 
...
--TODO:delete
		declare @pre_id int
		if @@bDelete = 0	set @pre_id = @csPreTsrEditDepositoBanco
		else							set @pre_id = @csPreTsrDeleteDepositoBanco
--TODO:delete

		-- Tiene permiso para editar depositos bancarios
		--
...
--TODO:delete
						if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar depositos bancarios'
						else							set @@editMsg = 'Usted no tiene permiso para borrar depositos bancarios'
--TODO:delete
	  ...
	  select @fca_id = fca_id from Documento where doc_id = @doc_id 
		...
		if not @fca_id is null begin
		  ...
		  if not exists(select fca_id from FechaControlAcceso 
		            where fca_id = @fca_id and @dbco_fecha between fca_fechaDesde and fca_fechaHasta) begin
