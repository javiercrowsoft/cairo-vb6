if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocFacturaCompraAsientoSaveIva]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocFacturaCompraAsientoSaveIva]
...
/*
 ...
 sp_DocFacturaCompraAsientoSaveIva 93

*/

go
create procedure sp_DocFacturaCompraAsientoSaveIva (
  @@fc_id             int,
  @@as_id             int,
  @@bIva_ri            tinyint,
  @@mon_id            int,
  ...
  if @@bIva_ri <> 0 begin
    ...
    if @@doc_esresumenbco <> 0 begin
      ...
      declare c_FacturaItemAsientoIva cursor for 
        ...
        select fci_ivari, fci_importe, fci_importeorigen, ti.cue_id
        from FacturaCompraItem fci inner join Producto p         on fci.pr_id            = p.pr_id
                                  inner join TasaImpositiva ti  on p.ti_id_ivariCompra  = ti.ti_id
      ...
      declare c_FacturaItemAsientoIva cursor for 
        ...
        select sum(fci_ivari), sum(fci_importe), sum(fci_importeorigen), ti.cue_id
        from FacturaCompraItem fci inner join Producto p         on fci.pr_id            = p.pr_id
                                  inner join TasaImpositiva ti  on p.ti_id_ivariCompra  = ti.ti_id
  
        where fc_id = @@fc_id
        group by    
                ti.cue_id having sum(fci_ivari) <> 0
    end
    ...
    if @@doc_esresumenbco <> 0 begin
      ...
      declare c_FacturaItemAsientoIva cursor for 
        ...
        select fci_ivarni, fci_importe, fci_importeorigen, ti.cue_id
        from FacturaCompraItem fci inner join Producto p         on fci.pr_id            = p.pr_id
                                  inner join TasaImpositiva ti  on p.ti_id_ivarniCompra = ti.ti_id
      ...
      declare c_FacturaItemAsientoIva cursor for 
        ...
        select sum(fci_ivarni), sum(fci_importe), sum(fci_importeorigen), ti.cue_id
        from FacturaCompraItem fci inner join Producto p         on fci.pr_id            = p.pr_id
                                  inner join TasaImpositiva ti  on p.ti_id_ivarniCompra = ti.ti_id
                                  
        where fc_id = @@fc_id
        group by    
                ti.cue_id having sum(fci_ivarni) <> 0
    end
