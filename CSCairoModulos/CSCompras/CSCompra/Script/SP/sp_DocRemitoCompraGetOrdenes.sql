if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocRemitoCompraGetOrdenes]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocRemitoCompraGetOrdenes]
...
exec sp_DocRemitoCompraGetOrdenes 4,2

*/

create procedure sp_DocRemitoCompraGetOrdenes (
  @@emp_id          int,
  @@prov_id         int,
  @@mon_id          int
)
...
declare @doct_Orden     int set @doct_Orden     = 35
  ...
  where 
    ...
    and   ((oc.oc_firmado <> 0 and d.doc_llevaFirma <> 0) or d.doc_llevaFirma = 0)
