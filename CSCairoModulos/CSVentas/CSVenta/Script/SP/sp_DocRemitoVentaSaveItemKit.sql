if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocRemitoVentaSaveItemKit]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocRemitoVentaSaveItemKit]

/*
 select * from RemitoVenta
 sp_DocRemitoVentaSaveItemKit 26

*/

go
create procedure sp_DocRemitoVentaSaveItemKit (
	@@rvTMP_id        int,
	@@rvi_id          int,
	@@st_id 					int,
	@@sti_orden				int out,
...
begin
  ...
  set nocount on
	...
	insert into StockItemKit (stik_id,stik_cantidad,pr_id,st_id,stik_llevanroserie)
									values   (@stik_id,@@rvi_cantidad,@@pr_id,@@st_id,@stik_llevanroserie)
	...
	begin
		...
		set @cantidad = @cantidad * @@rvi_cantidad 
...
ControlError:
	set @@MsgError = 'Ha ocurrido un error al grabar el item de stock del remito de venta. sp_DocRemitoVentaSaveItemKit.'
