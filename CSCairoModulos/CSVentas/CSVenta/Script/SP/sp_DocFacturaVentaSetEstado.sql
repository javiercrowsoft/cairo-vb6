if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocFacturaVentaSetEstado]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocFacturaVentaSetEstado]

/*

 sp_DocFacturaVentaSetEstado 21

*/

go
create procedure sp_DocFacturaVentaSetEstado (
  @@fv_id       int,
  @@Select      tinyint = 0,
  @@est_id      int = 0 out 
)
...
begin
  ...
  select @cli_id = cli_id, @firmado = fv_firmado, @est_id = est_id, @doc_id = doc_id
  ...
  if @est_id <> @estado_anulado begin
    ...
    if @firmado = 0 and @doc_llevafirma <> 0 and @est_id <> @estado_finalizado begin             
      ...
      select @deuda = round(sum(fvd_pendiente),2) from FacturaVentaDeuda where fv_id = @@fv_id
    ...
    update FacturaVenta set est_id = @est_id
    where fv_id = @@fv_id
  
  end

  exec sp_DocFacturaVentaSetEstadoCliente @@fv_id, @est_id out
...
ControlError:
  ...
  raiserror ('Ha ocurrido un error al actualizar el estado de la factura de venta. sp_DocFacturaVentaSetEstado.', 16, 1)
