if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_HojaRutaSaveTicketsAux]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_HojaRutaSaveTicketsAux]

go

create procedure sp_HojaRutaSaveTicketsAux (
  @@hr_id              int,
  @@mf_id_tickets     int out,
...
begin
  ...
  set @@mf_id_tickets   = isnull(@@mf_id_tickets,0)
...
------------------------------------------------------------------------------------------------------------
  ...
  exec sp_Cfg_GetValor  'Ventas-General',
                        'Mov. Fondo Tickets en Hoja Ruta',
                        @cfg_valor out,
                        0,
                        @@emp_id
  ...
  if @doc_id = 0 begin
    ...
    select 0 as result, 'Debe indicar un documento de movimientos de fondos para registrar la comision sobre tickets en rendiciones.' as info
    set @@bsuccess = 0
    return
  ...
  if @cue_id = 0 begin
    ...
    select 0 as result, 'Debe indicar una cuenta contable para registrar la comision sobre tickets en rendiciones.' as info
    set @@bsuccess = 0
    return
...
------------------------------------------------------------------------------------------------------------
  ...
  set @mf_total  = @@comision
  set @modifico = @@us_id

  select   @suc_id     = suc_id,
          @mf_fecha   = hr_fechaentrega,
          @mf_descrip = 'Generado por comision sobre tickets en hoja de ruta [' + hr_nrodoc + '] del ' + convert(varchar, hr_fecha, 105)
  ...
  -- // Talonario
    ...
    if @@mf_id_tickets <> 0 select @mfi_id = mfi_id from MovimientoFondoItem where mf_id = @@mf_id_tickets
...
------------------------------------------------------------------------------------------------------------
  ...
  -- 2 llamo al spDocMovimientoFondoSave
    ...
    if @@bSuccess <> 0 begin
      ...
      update HojaRuta set mf_id_tickets = @mf_id where hr_id = @@hr_id

      set @@mf_id_tickets = @mf_id
      set @@mf_nrodoc     = @mf_nrodoc
