if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocRemitoVentaSetEstado]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocRemitoVentaSetEstado]

/*

 sp_DocRemitoVentaSetEstado 21

*/

go
create procedure sp_DocRemitoVentaSetEstado (
  @@rv_id       int,
  @@Select      tinyint = 0,
  @@est_id      int = 0 out 
)
...
begin
  ...
  select @cli_id = cli_id, @firmado = rv_firmado, @est_id = est_id, @doc_id = doc_id
  ...
  if @est_id <> @estado_anulado begin
    ...
    if @firmado = 0 and @doc_llevafirma <> 0 and @est_id <> @estado_finalizado begin             
      ...
      select @deuda = round(sum(rvi_pendientefac),2) from RemitoVentaItem where rv_id = @@rv_id
    ...
    update RemitoVenta set est_id = @est_id
    where rv_id = @@rv_id
...
ControlError:
  ...
  raiserror ('Ha ocurrido un error al actualizar el estado del Remito de venta. sp_DocRemitoVentaSetEstado.', 16, 1)
