if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocOrdenServicioEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocOrdenServicioEditableGet]

go
/*

sp_DocOrdenServicioEditableGet 57,7,0,'',1

*/

create procedure sp_DocOrdenServicioEditableGet (
  @@emp_id        int,
  @@os_id         int,
  @@us_id         int,
...
begin
  ...
  declare @csTickEditOrdenServ     int set @csTickEditOrdenServ     = 28001
  declare @csTickDeleteOrdenServ  int set @csTickDeleteOrdenServ  = 28002
  ...
  if @@os_id <> 0 begin
    ...
    from OrdenServicio c inner join Documento d on c.doc_id = d.doc_id 
    where os_id = @@os_id
...
--TODO:delete
    declare @pre_id int
    if @@bDelete = 0  set @pre_id = @csTickEditOrdenServ
    else              set @pre_id = @csTickDeleteOrdenServ
--TODO:delete

    -- Tiene permiso para editar ordenes de servicio
    --
...
--TODO:delete
            if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar ordenes de servicio'
            else              set @@editMsg = 'Usted no tiene permiso para borrar ordenes de servicio'
--TODO:delete
...
--TODO:delete
            if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar ' + isnull(@doc_nombre,'')
            else              set @@editMsg = 'Usted no tiene permiso para borrar ' + isnull(@doc_nombre,'')
--TODO:delete
    ...
    if not @fca_id is null begin
      ...
      if not exists(select fca_id from FechaControlAcceso 
                where fca_id = @fca_id and @os_fecha between fca_fechaDesde and fca_fechaHasta) begin
    ...
    if exists(select os_id from OrdenRemitoVenta r inner join OrdenServicioitem osi on r.osi_id = osi.osi_id where os_id = @@os_id) begin
      set @@bEditable = 0
      set @@editMsg = 'El comprobante esta vinculado a un parte de entrega'
      if @@ShowMsg <> 0
        select [Editable]=@@bEditable, [EditMsg]= @@editMsg
      return
    end
