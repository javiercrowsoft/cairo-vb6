if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocParteReparacionSaveItemKit]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocParteReparacionSaveItemKit]

/*
 select * from ParteReparacion
 sp_DocParteReparacionSaveItemKit 26

*/

go
create procedure sp_DocParteReparacionSaveItemKit (
	@@prpTMP_id       int,
	@@prpi_id         int,
	@@st_id 					int,
	@@sti_orden				int out,
...
begin
  ...
  set nocount on
	...
	insert into StockItemKit (stik_id,stik_cantidad,pr_id,st_id,stik_llevanroserie)
									values   (@stik_id,@@prpi_cantidad,@@pr_id,@@st_id,@stik_llevanroserie)
	...
	begin
		...
		set @cantidad = @cantidad * @@prpi_cantidad 
...
ControlError:
	set @@MsgError = 'Ha ocurrido un error al grabar el item de stock del parte de reparación. sp_DocParteReparacionSaveItemKit.'
