if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocLiquidacionEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocLiquidacionEditableGet]

go
/*

sp_DocLiquidacionEditableGet 57,7,0,'',1

*/

create procedure sp_DocLiquidacionEditableGet (
  @@emp_id        int,
  @@liq_id         int,
  @@us_id         int,
...
begin
  ...
  declare @csPrePEditLiquidacion     int set @csPrePEditLiquidacion     = 35038
  declare @csPrePDeleteLiquidacion  int set @csPrePDeleteLiquidacion  = 35039
  ...
  if @@liq_id <> 0 begin
    ...
    from Liquidacion c inner join Documento d on c.doc_id = d.doc_id 
    where liq_id = @@liq_id
...
--TODO:delete
    declare @pre_id int
    if @@bDelete = 0  set @pre_id = @csPrePEditLiquidacion
    else              set @pre_id = @csPrePDeleteLiquidacion
--TODO:delete

    -- Tiene permiso para editar liquidaciones de haberes
    --
...
--TODO:delete
            if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar liquidaciones de haberes'
            else              set @@editMsg = 'Usted no tiene permiso para borrar liquidaciones de haberes'
--TODO:delete
...
--TODO:delete
    ...
    if not @fca_id is null begin
      ...
      if not exists(select fca_id from FechaControlAcceso 
                where fca_id = @fca_id and @liq_fecha between fca_fechaDesde and fca_fechaHasta) begin
