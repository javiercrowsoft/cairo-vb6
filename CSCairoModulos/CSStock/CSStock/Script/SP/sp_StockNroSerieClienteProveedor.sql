...
if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_StockNroSerieClienteProveedor]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_StockNroSerieClienteProveedor]
go

-- sp_StockNroSerieClienteProveedor 4193

create procedure sp_StockNroSerieClienteProveedor (
 @@prns_id int = 0
)
as
...
begin
  ...
  declare c_prns_prov insensitive cursor for
    select prns_id from ProductoNumeroSerie where prns_id = @@prns_id or @@prns_id = 0
