if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocStockClienteEditableGet]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocStockClienteEditableGet]

go
/*

sp_DocStockClienteEditableGet 8,7,0,'',1

*/

create procedure sp_DocStockClienteEditableGet (
	@@emp_id    		int,
	@@stcli_id 			int,
  @@us_id     		int,
...
begin
  ...
  declare @csPreStEditStockCliente 		int set @csPreStEditStockCliente 		= 20020
	declare @csPreStDeleteStockCliente	int set @csPreStDeleteStockCliente	= 20021
	...
	if @@stcli_id <> 0 begin
	  ...
	  from StockCliente c inner join Documento d on c.doc_id = d.doc_id 
...
--TODO:delete
		declare @pre_id int
		if @@bDelete = 0	set @pre_id = @csPreStEditStockCliente
		else							set @pre_id = @csPreStDeleteStockCliente
--TODO:delete
...
--TODO:delete
						if @@bDelete = 0  set @@editMsg = 'Usted no tiene permiso para editar transferencias de stock a cliente'
						else							set @@editMsg = 'Usted no tiene permiso para borrar transferencias de stock a cliente'
--TODO:delete
	  ...
	  select @fca_id = fca_id from Documento where doc_id = @doc_id 
		...
		if not @fca_id is null begin
		  ...
		  if not exists(select fca_id from FechaControlAcceso 
		            where fca_id = @fca_id and @stcli_fecha between fca_fechaDesde and fca_fechaHasta) begin
