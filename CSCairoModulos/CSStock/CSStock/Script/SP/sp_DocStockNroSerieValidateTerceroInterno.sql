if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocStockNroSerieValidateTerceroInterno]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocStockNroSerieValidateTerceroInterno]

/*

exec sp_docstockcachecreate
exec sp_DocStockNroSerieValidateTerceroInterno 0,91457
...
go
create procedure sp_DocStockNroSerieValidateTerceroInterno (
	@@pr_id 		int = 0,
	@@prns_id 	int = 0
)
as
...
ControlError:
	...
	set @MsgError = 'Ha ocurrido un error al validar los numeros de serie. sp_DocStockNroSerieValidateTerceroInterno. ' + IsNull(@MsgError,'')
