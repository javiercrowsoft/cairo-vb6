if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_DocStockClienteStockSave]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_DocStockClienteStockSave]

/*

begin transaction

 sp_DocStockClienteStockSave 1
...
go
create procedure sp_DocStockClienteStockSave (
	@@stcli_id        int,
	@@stTMP_id 				int,
	@@st_id           int out,
...
begin
  ...
  declare @orden          smallint
	...
	select  @st_doc_cliente 	= stcli_nrodoc + ' ' + cli_nombre,
				  @doct_id_cliente 	= stcli.doct_id,
					@id_cliente       = stcli.stcli_id
	from StockCliente stcli inner join Cliente cli on stcli.cli_id = cli.cli_id
...
ControlError:
	...
	set @@MsgError = 'Ha ocurrido un error al grabar la transferencia de stock a cliente. sp_DocStockClienteStockSave.'
...
Roll:
	...
	-- El rollback lo hace sp_DocStockClienteSave
