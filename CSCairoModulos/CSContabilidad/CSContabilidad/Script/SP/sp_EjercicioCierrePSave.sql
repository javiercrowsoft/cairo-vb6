if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_EjercicioCierrePSave]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_EjercicioCierrePSave]

--  sp_EjercicioCierrePSave 1,1,0
-- 

go
create procedure sp_EjercicioCierrePSave (                                                                  
...
begin
  ...
  if @as_id_cr is null begin

    raiserror ('@@ERROR_SP:Antes de generar el cierre patrimonial debe generar el cierre de resultados.', 16, 1)
    return
    ...
    insert into AsientoTMP( asTMP_id,
                      ...
                      values
                          ( @asTMP_id,
                            ...
                            0,
                            @as_nrodoc,
                            'Cierre Patrimonial del Ejercicio ' + @ejc_nombre,
                            @fechaFin,
                            @doc_id,
                            @doct_id,
                            @@us_id
    ...
    if @@bCentroCosto <> 0 begin
      ...
      declare c_items insensitive cursor for
        ...
        where cuec_id <> 9 and cuec_id <> 10 /*Sin cuentas de resultado*/
    ...
    begin tran
      ...
      exec sp_DocAsientoSave @asTMP_id, @as_id_cp out, 0
      if @@error <> 0 goto ControlError
    
      update EjercicioContable set as_id_cierrepatrimonial = @as_id_cp where ejc_id = @@ejc_id
...
ControlError:
  ...
  raiserror ('Ha ocurrido un error al grabar el asiento de cierre patrimonial. sp_EjercicioCierrePSave.', 16, 1)
