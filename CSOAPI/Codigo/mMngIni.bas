...
' funciones publicas
#If Not PREPROC_CAIRO_LAUNCH Then
...
Public Sub SaveIniValue(ByVal Section, ByVal Item As String, ByVal Value As Variant, ByVal File As String)
  ...
  GoTo ExitProc
ControlError:
  MngError Err, "SaveIniValue", C_Module, vbNullString
  If Err.Number Then Resume ExitProc
ExitProc:
  On Error Resume Next
End Sub
...
#If Not PREPROC_CAIRO_LAUNCH Then
  ...
  Public Function GetFullIniFileName() As String
    GetFullIniFileName = pGetFullIniFile()
  End Function

  Private Function pGetFullIniFile() As String
    Dim FullFileName As String
    
#If Not PREPROC_CSSERVER Then
    Dim File         As CSKernelFile.cFileEx
    
    Set File = New CSKernelFile.cFileEx
    
    FullFileName = GetValidPath(File.GetWindowsDir()) & c_MainIniFile
    If Not File.FileExists(FullFileName) Then
#End If

  #If PREPROC_ABMGENERIC Then
      If LenB(gAppPath) = 0 Then gAppPath = App.Path
      FullFileName = gAppPath & "\" & c_MainIniFile
  #ElseIf PREPROC_CSGENERALEX Then
      FullFileName = CSKernelClient2.AppPath & "\" & c_MainIniFile
  #Else
...
#If Not (PREPROC_CSDATABASE_CLIENT Or _
         PREPROC_CSDATABASE Or _
         PREPROC_ABMGENERIC Or _
         PREPROC_CSGENERALEX) Then
