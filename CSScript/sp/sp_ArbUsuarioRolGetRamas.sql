...
if exists (select * from sysobjects where id = object_id(N'[dbo].[sp_ArbUsuarioRolGetRamas]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[sp_ArbUsuarioRolGetRamas]


/*

select * from rol where rol_nombre = 'Ventas-Alem-Retiro'
select * from arbol where arb_nombre='usuarios'

exec sp_ArbUsuarioRolGetRamas 0,6,187

*/

go
create procedure sp_ArbUsuarioRolGetRamas (
	@@us_id  			int,
	@@rol_id 			int,
  @@arb_id 	    int
)
...
begin
	...
	delete hoja
	from arbol
	where not exists(select us_id from usuario where us_id = id)
		and hoja.arb_id = @@arb_id
		and hoja.arb_id = arbol.arb_id
    and arbol.tbl_id = 3 /* Usuario */
	...
	if @@us_id <> 0 begin
		...
		where arb_id = @@arb_id
			...
			-- No existen roles en esta rama sin estar asociados a este usuario
			and not exists (select * from hoja 
											where  ram_id = rama.ram_id 
												 and not exists(select * from usuariorol where rol_id = id
																																	 and us_id  = @@us_id
																				)
										)
...
--///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		where arb_id = @@arb_id
			-- Solo ramas sin subramas (sin hijos)
			and not exists (select * from rama r where r.arb_id = @@arb_id and r.ram_id_padre = rama.ram_id)
			-- No existen usuarios en esta rama sin estar asociados a este rol
			and not exists (select * from hoja 
											where  ram_id = rama.ram_id 
												 and not exists(select * from usuariorol where us_id  = id
																																	 and rol_id = @@rol_id
																		   )
										)
			-- Tiene que tener hojas
