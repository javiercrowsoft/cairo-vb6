if exists (select * from sysobjects where id = object_id(N'[dbo].[frTransferenciaStock]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[frTransferenciaStock]
go
...
exec frTransferenciaStock 3564
select * from stockitem where st_id = 3564 order by stik_id

frTransferenciaStock 2
select * from stock

*/

create procedure frTransferenciaStock (
...
-- Numeros de serie
	...
	declare @lst_series varchar(8000)
	set @lst_series = ''
