...
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_9990]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_9990]


go
create procedure DC_CSC_VEN_9990 (
...
--  exec sp_ArbGetGroups @ram_id_cuenta, @clienteID, @@us_id
  ...
  update FacturaVenta set fv_grabarAsiento = 1
  where 
     ...
     and (
      ...
      and (     (@cue_id = 0 or @ram_id_cuenta = 0)
            and  exists(select * from AsientoItem 
                       where as_id = FacturaVenta.as_id 
  ...
  select fv_id,'20040304' from FacturaVenta 
  where fv_grabarAsiento <> 0 

  exec sp_DocFacturaVentaAsientosSave 
