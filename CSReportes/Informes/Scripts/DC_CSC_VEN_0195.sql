...
DC_CSC_VEN_0195 1, '20050101','20100201','0', '0','0','0','0','0','0'


*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0195]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0195]

go
create procedure DC_CSC_VEN_0195 (
...
select 
    isnull(isnull(pro0.pro_id,pro1.pro_id),pro2.pro_id),
		1 																				as orden_id,
  	IsNull(isnull(pro0.pro_codigo,pro1.pro_codigo),pro2.pro_codigo)		
																							as Codigo,
  	IsNull(IsNull(isnull(pro0.pro_nombre,pro1.pro_nombre),pro2.pro_nombre)
...
from 
  ...
  facturaventa fv inner join cliente   cli         on fv.cli_id   = cli.cli_id 
                  ...
                  inner join empresa   emp         on doc.emp_id  = emp.emp_id
									...
									left join transporte trans       on fv.trans_id = trans.trans_id
																											and cli_pciaTransporte <> 0

									left join provincia   pro0       on trans.pro_id 			= pro0.pro_id

           	      left join provincia   pro1       on fv.pro_id_destino = pro1.pro_id
           	      left join provincia   pro2       on cli.pro_id  			= pro2.pro_id
where 
...
group by 
    IsNull(isnull(pro0.pro_id,pro1.pro_id),pro2.pro_id),
    IsNull(isnull(pro0.pro_codigo,pro1.pro_codigo),pro2.pro_codigo),
    IsNull(IsNull(IsNull(pro0.pro_nombre,pro1.pro_nombre),pro2.pro_nombre),'Clientes sin provincia') 
