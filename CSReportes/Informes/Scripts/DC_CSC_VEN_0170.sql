...
---------------------------------------------------------------------
*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0170]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0170]

go
create procedure DC_CSC_VEN_0170 (
...
select 
    1                                   as orden_id,
    pr_nombreventa                      as [Nombre Compra],
    pr_nombrecompra                      as [Nombre Venta],
    pr_codigo                            as Codigo,
    case pr_llevastock
      when 0 then 'no'
      else         'si'
