...
exec DC_CSC_VEN_0610 1,'20060501','20070430','0','0','0',0,'1',0

*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0610]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0610]

go
create procedure DC_CSC_VEN_0610 (
...
begin
	...
	create table #t_dc_csc_VEN_0610 (cue_id int)

	insert into #t_dc_csc_VEN_0610 (cue_id)
	...
	where fv_fecha between @@Fini and @@Ffin
		and est_id <> 7
		and not exists(select * from #t_dc_csc_VEN_0610 
									where cue_id = isnull(isnull(cue.cue_id,
																								clicueg.cue_id),
																				 cueg.cue_id)
								)

--	select cue_nombre,cue.cue_id from #t_dc_csc_VEN_0610 t inner join cuenta cue on t.cue_id = cue.cue_id
...
where 
			...
			and exists(select * from #t_dc_csc_VEN_0610 where cue_id = cue.cue_id)
...
order by cue_nombre, cue_codigo, [de Ventas], tercero, Fecha, [Comp. Origen]
