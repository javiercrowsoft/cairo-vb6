/*---------------------------------------------------------------------
Nombre: Ranking de por Articulos (facturados)
---------------------------------------------------------------------*/
/*  
...
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0300]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0300]

go
create procedure DC_CSC_VEN_0300 (
...
create table #t_dc_csc_ven_0300 (
                                  pr_id int,
                                  Cantidad  decimal(18,6)
      )

insert into #t_dc_csc_ven_0300 (pr_id, Cantidad)


select top 100
       pr.pr_id,
       sum(fvi_cantidad)  as Cantidad
...
set @sqlstmt = 'select top ' + convert(varchar,@@top)
             + ' 1 as orden_id,t.pr_id,pr_nombreVenta as Articulo, pr_codigo as Codigo, Cantidad '
             + 'from #t_dc_csc_ven_0300 t inner join Producto pr on t.pr_id = pr.pr_id '
             + 'order by Cantidad desc'
