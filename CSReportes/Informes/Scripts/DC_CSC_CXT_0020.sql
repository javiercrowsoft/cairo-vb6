...
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_CXT_0020]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_CXT_0020]
go



create procedure DC_CSC_CXT_0020 (
...
select 
      ...
      impt_despachonro        as [Despacho Nro],
      impt_fechaoficial       as [Fecha Oficializado],
      impt_seguro             as [Seguro],
      impt_flete              as [Flete],
      ...
      lgj_codigo          as [Legajo Codigo],
      lgj_titulo          as [Legajo Titulo],
...
from 
      ImportacionTemp  inner join documento     on ImportacionTemp.doc_id   = documento.doc_id
                       ...
                       left join legajo  lgj    on ImportacionTemp.lgj_id   = lgj.lgj_id
where 
