...
exec [DC_CSC_VEN_0570] 1,'20070101 00:00:00','20080715 00:00:00','0','0','0','0','0','0','0',3,1


*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0570]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0570]

go
create procedure DC_CSC_VEN_0570 (
...
create table #t_DC_CSC_VEN_0570 ( ven_id 			int,
																	pa_id     	int,
...
create table #t_DC_CSC_VEN_0570_2(ven_id 			int not null,
																	pa_id     	int,
																	pa_id_orden int not null default(0),
																	cli_id    	int not null,
																	pr_id				int not null,
...
insert into #t_DC_CSC_VEN_0570_2
															(
																	ven_id,
																	cli_id,
																	pr_id,
...
--
	...
	--
	update #t_DC_CSC_VEN_0570_2 set 
...
--
		...
		update #t_DC_CSC_VEN_0570 set pa_id = pro.pa_id
		from Cliente cli  inner join Provincia pro on cli.pro_id = pro.pro_id
											inner join pais pa       on pro.pa_id  = pa.pa_id
		where #t_DC_CSC_VEN_0570.cli_id = cli.cli_id
		
		update #t_DC_CSC_VEN_0570 set total_vendedor = (select sum(total) from #t_DC_CSC_VEN_0570 t where isnull(ven_id,0) = isnull(#t_DC_CSC_VEN_0570.ven_id,0))
		update #t_DC_CSC_VEN_0570 set total_pais = (select sum(total) from #t_DC_CSC_VEN_0570 t where isnull(pa_id,0) = isnull(#t_DC_CSC_VEN_0570.pa_id,0))
		
		declare @total decimal(18,6)
		
		select @total = sum(total) from #t_DC_CSC_VEN_0570
...
--
	...
	declare c_pais insensitive cursor for select pa_id from #t_DC_CSC_VEN_0570 group by pa_id order by sum(total) desc
	open c_pais
	...
	begin
		...
		update #t_DC_CSC_VEN_0570 set pa_id_orden = @pa_id_orden where isnull(pa_id,0) = isnull(@pa_id,0)
...
--
		select 
					...
					sum(Neto)								as Neto,
					sum(IVA)								as IVA,
					sum([Otros Impuestos])	as [Otros Impuestos],
					sum(Total)							as Total,
		...
		from #t_DC_CSC_VEN_0570 t left join pais pa on t.pa_id = pa.pa_id
	...
	order by orden_id desc, vendedor, empresa, producto, total_pais desc, total desc, pa_id_orden
