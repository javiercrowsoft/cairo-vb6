...
---------------------------------------------------------------------*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_CON_0147]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_CON_0147]
...
select * from hoja where arb_id = 56 and id = 153
 ...
 [DC_CSC_CON_0147] 7,'20061201 00:00:00','20061231 00:00:00','0','0','N53442','0','1',217

*/

create procedure DC_CSC_CON_0147 (
...
create table #DC_CSC_CON_0147_cuentas (
																				nodo_id int,
																				nodo_2 int,
																				nodo_3 int,
																				nodo_4 int,
...
while exists(select * from rama r
						 where  arb_id = @@arb_id
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_2 = r.ram_id)
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_3 = r.ram_id)
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_4 = r.ram_id)
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_5 = r.ram_id)
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_6 = r.ram_id)
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_7 = r.ram_id)
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_8 = r.ram_id)
								and not exists (select * from #DC_CSC_CON_0147_cuentas where nodo_9 = r.ram_id)
...
begin
	...
	if @n = 2 begin
		...
		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2) 
		select ram_id, ram_id from rama where ram_id_padre = @raiz

	end else begin if @n = 3 begin

		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2, nodo_3) 
		select ram_id, nodo_2, ram_id 
		from rama r inner join #DC_CSC_CON_0147_cuentas n on r.ram_id_padre = n.nodo_2

	end else begin if @n = 4 begin

		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2, nodo_3, nodo_4) 
		select ram_id, nodo_2, nodo_3, ram_id
		from rama r inner join #DC_CSC_CON_0147_cuentas n on r.ram_id_padre = n.nodo_3

	end else begin if @n = 5 begin

		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2, nodo_3, nodo_4, nodo_5) 
		select ram_id, nodo_2, nodo_3, nodo_4, ram_id
		from rama r inner join #DC_CSC_CON_0147_cuentas n on r.ram_id_padre = n.nodo_4

	end else begin if @n = 6 begin

		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2, nodo_3, nodo_4, nodo_5, nodo_6) 
		select ram_id, nodo_2, nodo_3, nodo_4, nodo_5, ram_id
		from rama r inner join #DC_CSC_CON_0147_cuentas n on r.ram_id_padre = n.nodo_5

	end else begin if @n = 7 begin

		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2, nodo_3, nodo_4, nodo_5, nodo_6, nodo_7) 
		select ram_id, nodo_2, nodo_3, nodo_4, nodo_5, nodo_6, ram_id
		from rama r inner join #DC_CSC_CON_0147_cuentas n on r.ram_id_padre = n.nodo_6

	end else begin if @n = 8 begin

		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2, nodo_3, nodo_4, nodo_5, nodo_6, nodo_7, nodo_8) 
		select ram_id, nodo_2, nodo_3, nodo_4, nodo_5, nodo_6, nodo_7, ram_id
		from rama r inner join #DC_CSC_CON_0147_cuentas n on r.ram_id_padre = n.nodo_7

	end else begin if @n = 9 begin

		insert #DC_CSC_CON_0147_cuentas (nodo_id, nodo_2, nodo_3, nodo_4, nodo_5, nodo_6, nodo_7, nodo_8, nodo_9) 
		select ram_id, nodo_2, nodo_3, nodo_4, nodo_5, nodo_6, nodo_7, nodo_8, ram_id
		from rama r inner join #DC_CSC_CON_0147_cuentas n on r.ram_id_padre = n.nodo_8
...
select  
			  ...
			  cuec_nombre     					as Categoria,
				...
				case 
						when sum(asi_debe)-sum(asi_haber) > 0 then sum(asi_debe)-sum(asi_haber)
						else 0
				end												as Debe, 

				case 
						when sum(asi_debe)-sum(asi_haber) < 0 then sum(asi_haber)-sum(asi_debe)
						else 0
				end												as Haber
...
from
				 ...
				 asiento ast     inner join asientoitem 			asi  	on ast.as_id   = asi.as_id
                         ...
                         left  join Documento 				doccl	on ast.doc_id_cliente	= doccl.doc_id
												 ...
												 left  join #DC_CSC_CON_0147_cuentas nodo on h.ram_id = nodo.nodo_id
