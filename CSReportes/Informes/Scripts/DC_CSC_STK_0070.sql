...
Nombre: 
---------------------------------------------------------------------*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_STK_0070]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_STK_0070]
...
create procedure DC_CSC_STK_0070 (
...
-- Discrimino depositos internos
      ...
      and   (sti.pr_id_kit = @pr_id or @pr_id=0)
      and   (d.depl_id = @depl_id or @depl_id=0)
      and   (df.depf_id = @depf_id or @depf_id=0)
      and   (s.suc_id = @suc_id or @suc_id=0)
...
and   (
          (exists(select rptarb_hojaid 
                  ...
                  where
                       rptarb_cliente = @clienteID
                  and  tbl_id = 30 
                  and  rptarb_hojaid = sti.pr_id_kit
                 ) 
           )
        or 
           (@ram_id_Producto = 0)
