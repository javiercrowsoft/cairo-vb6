...
*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_CON_0220]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_CON_0220]

/*

select * from circuitocontable

DC_CSC_CON_0220 1,'20050107','20050107','1','2'


*/
go
create procedure DC_CSC_CON_0220 (
...
select 
      ...
      cue_identificacionexterna      as [Codigo Concepto],
      cue_nombre                    as Concepto,
      ...
      0                                          as [Importe interno],
      cue_identificacionexterna + ' ' + cue_nombre
                                                as Concepto,
...
from FacturaCompra fc     inner join FacturaCompraItem fci        on fc.fc_id      = fci.fc_id
                          ...
                          inner join Empresa                      on d.emp_id      = empresa.emp_id 
                          inner join Proveedor p                  on fc.prov_id    = p.prov_id
                          inner join CuentaGrupo cueg             on d.cueg_id     = cueg.cueg_id

                          left  join ProveedorCuentaGrupo pcueg   on     cueg.cueg_id   = pcueg.cueg_id
                                                                    and p.prov_id      = pcueg.prov_id

                          left  join Cuenta cue                   on isnull(pcueg.cue_id,cueg.cue_id) = cue.cue_id
