...
*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_STK_0090]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_STK_0090]

GO

create procedure DC_CSC_STK_0090 (
...
having   (sum(sti_ingreso)  - sum(sti_salida)) < pr_stockminimo
...
having   ((sum(sti_ingreso) - sum(sti_salida))/pr_kitItems) < pr_stockminimo
GO