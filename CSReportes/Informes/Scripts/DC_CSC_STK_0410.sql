...
---------------------------------------------------------------------*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_STK_0410]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_STK_0410]
...
create procedure [dbo].[DC_CSC_STK_0410] (
...
select 
        ...
        isnull(prov_nombre+' '+prov_tel,'(Sin Proveedor)') as Proveedor
from
      ...
      Stock   inner join StockItem sti              on Stock.st_id     = sti.st_id
              ...
              left  join ProductoProveedor prprov   on p.pr_id         = prprov.pr_id
...
group by     
        ...
        pr_stockmaximo,
        un_nombre,
        isnull(prov_nombre+' '+prov_tel,'(Sin Proveedor)')
...
order by isnull(prov_nombre+' '+prov_tel,'(Sin Proveedor)')
