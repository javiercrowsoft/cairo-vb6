...
/*  
 ...
 [DC_CSC_VEN_0625] 79,'1','N82617','102','Todo en Cartuchos','http://www.todoencartuchos.com/catalog/lib/index.php?pr_id=','http://www.todoencartuchos.com/catalog/lib/images/','Contado','Cartuchos para Impresoras','Segun stock del dia de la compra',-1
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0625]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0625]
GO


create procedure DC_CSC_VEN_0625 (
...
begin
	...
	select c.pr_id,
				 ...
				 dbo.str_from_iso_to_acute(
						 replace(
										 case when pr_nombreweb <> '' then pr_nombreweb
													else 												 pr_nombreventa
										 end, '&','&amp;')
						),' ','%20%')
													as Link,
				'ARS'             as disponibilidad,
				 c.precio   			as Precio,
