/*---------------------------------------------------------------------
Nombre: Costo de Mercaderías Vendidas por remitos
...
DC_CSC_VEN_0357 1, '20060101','20061231','0', '106','0','0','0','0','0',29,2,1

*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0357]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0357]

go
create procedure DC_CSC_VEN_0357 (
...
create table #t_DC_CSC_VEN_0357(pr_id 		      int not null, 
...
insert into #t_DC_CSC_VEN_0357 (pr_id, pr_esKit, pr_ventacompra, cantidad, venta)
		    		...
		    		sum (
						...
						sum (
						        ...
						        when 24  then - (rvi_neto
																				- (rvi_neto * rv_descuento1 / 100)
																				- (
																						(
																							rvi_neto - (rvi_neto * rv_descuento1 / 100)
																						) * rv_descuento2 / 100
																					)
																		)	
						        else         rvi_neto
...
----------------------------------------------------------------------------------------
	...
	-- Para resolver Kits
	--
  create table #t_DC_CSC_VEN_0357_i (pr_id int not null, costo decimal(18,6) not null)
...
declare c_precios insensitive cursor for select pr_id, pr_esKit, pr_ventacompra from #t_DC_CSC_VEN_0357
...
--
      ...
      if @pr_esKit <> 0 begin
        ...
        --
          ...
          select @costo_item = costo from #t_DC_CSC_VEN_0357_i where pr_id = @pr_id_item
          ...
          if @costo_item is null begin
            ...
            insert into #t_DC_CSC_VEN_0357_i (pr_id, costo) values (@pr_id_item, @costo_item)

          end

          if @@bShowInsumo <> 0 begin
            insert into  #t_DC_CSC_VEN_0357 (pr_id,  pr_esKit, pr_ventacompra, pr_id_insumo , cantidad, costo)
...
--//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  set @costo = IsNull(@costo,0) / @pr_ventacompra
	...
	update #t_DC_CSC_VEN_0357 set costo = @costo where pr_id = @pr_id and pr_id_insumo is null
