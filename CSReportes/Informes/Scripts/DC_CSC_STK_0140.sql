...
DC_CSC_STK_0140 1, 
								...
								'0'
*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_STK_0140]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_STK_0140]

go
create procedure DC_CSC_STK_0140 (
...
select 
              ...
              st.st_numero          [Trans. Numero],
							...
							case
								when @depl_id = 0 									then sum (sti_ingreso)
								when st.depl_id_origen  = @depl_id	then -sum (sti_ingreso)
								when st.depl_id_destino = @depl_id 	then sum (sti_ingreso)
							end							     [Cantidad]
...
--																				and sti_ingreso <> 0
	         ...
	         left join moneda    mon          on doc2.mon_id   = mon.mon_id
where
