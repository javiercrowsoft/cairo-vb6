...
Nombre: 
---------------------------------------------------------------------*/
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[DC_CSC_STK_0100]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_STK_0100]
GO
...
create  procedure DC_CSC_STK_0100 (
...
having   (sum(sti_ingreso)  - sum(sti_salida)) < pr_reposicion
...
having   ((sum(sti_ingreso) - sum(sti_salida))/pr_kitItems) < pr_reposicion
