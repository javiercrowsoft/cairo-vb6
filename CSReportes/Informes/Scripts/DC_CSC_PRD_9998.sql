...
---------------------------------------------------------------------*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_PRD_9998]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_PRD_9998]

/*

 select * from TmpStringToTable

 DC_CSC_PRD_9998 1,'', 'e10452,e10453,e10454,e10455,e10456,e10457,e10458,e10459'

 DC_CSC_PRD_9998 1, '2739696'

*/

go
create procedure DC_CSC_PRD_9998 (
...
create table #t_DC_CSC_PRD_9998 (pr_id int, cue_id int)
...
begin
  ...
  if not exists (select cueg_id from CuentaGrupo where cueg_codigo = 'V-' + @cue_identificacionexterna) begin

    insert into CuentaGrupo (cueg_id, cueg_nombre, cueg_codigo, cue_id, modifico)
                  values    (@cueg_id, 'V-' + @cue_nombre, 'V-' + @cue_identificacionexterna, @cue_id, @@us_id)
    ...
    insert into Producto (pr_id, pr_nombrecompra, pr_nombreventa, pr_sevende, pr_codigo, ti_id_ivariventa, ti_id_ivarniventa, cueg_id_venta, un_id_venta, modifico)
                  values (@pr_id, @cue_nombre, @cue_nombre, 1, @cue_identificacionexterna, @ti_idri, @ti_idrni, @cueg_id, @un_id, @@us_id)

    insert into #t_DC_CSC_PRD_9998 (pr_id) values (@pr_id)

  end else

    insert into #t_DC_CSC_PRD_9998 (cue_id) values (@cue_id)
...
from #t_DC_CSC_PRD_9998 t left join cuenta cue on t.cue_id = cue.cue_id
