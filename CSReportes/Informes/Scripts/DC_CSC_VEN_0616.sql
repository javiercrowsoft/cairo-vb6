/*---------------------------------------------------------------------
Nombre: Imputacion Contable de Documentos de Venta (El saldo inicial es por cliente)
---------------------------------------------------------------------*/
/*  
...
exec DC_CSC_VEN_0616 1,'20060501','20070430','0','0','0',0,'1',0

*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_VEN_0616]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_VEN_0616]

go
create procedure DC_CSC_VEN_0616 (
...
begin
	...
	create table #t_DC_CSC_VEN_0616_as (as_id int)

	if @@bSinAsEj <> 0 begin
		insert into #t_DC_CSC_VEN_0616_as(as_id)
	...
	create table #t_DC_CSC_VEN_0616 (cue_id int)

	insert into #t_DC_CSC_VEN_0616 (cue_id)
	...
	where fv_fecha between @@Fini and @@Ffin
		and est_id <> 7
		and not exists(select * from #t_DC_CSC_VEN_0616 
									where cue_id = isnull(isnull(cue.cue_id,
																								clicueg.cue_id),
																				 cueg.cue_id)
								)

--	select cue_nombre,cue.cue_id from #t_DC_CSC_VEN_0616 t inner join cuenta cue on t.cue_id = cue.cue_id
...
where 
			...
			and exists(select * from #t_DC_CSC_VEN_0616 where cue_id = cue.cue_id)

			and not exists(select * from #t_DC_CSC_VEN_0616_as where as_id = ast.as_id)
