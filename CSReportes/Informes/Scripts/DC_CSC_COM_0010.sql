...
[DC_CSC_COM_0010] 1,'20050101 00:00:00','20071231 00:00:00','0','0','0',1,'0',0
...
DC_CSC_COM_0010 
 ...
 [DC_CSC_COM_0010] 1,'20050101 00:00:00','20051231 00:00:00','0','0','0','1',-1,'2',3


*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_COM_0010]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_COM_0010]

go
create procedure [dbo].[DC_CSC_COM_0010] (
...
--//////////////////////////////////////////
		...
		insert into #dc_csc_com_0010 (prov_id,cue_id,emp_id,suc_id,pago,pendiente)
...
--//////////////////////////////////////////
		...
		insert into #dc_csc_com_0010 (prov_id,cue_id,emp_id,suc_id,neto,descuento,subtotal,iva,total,pago,pendiente)
		...
		if @@conRemito <> 0 begin
			...
			select 
			        ...
			        suc_id,
							...
							case rc.doct_id 
								when 25 then			- (isnull(rc_importedesc1,0) 
															   + isnull(rc_importedesc2,0)
				                        )
								else             isnull(rc_importedesc1,0) 
															   + isnull(rc_importedesc2,0)
							end                
														 as [Descuento],
				      ...
				      case rc.doct_id 
								when 25	then   - (isnull(rc_ivari,0)
															  + isnull(rc_ivarni,0) 
																)
								else              isnull(rc_ivari,0)
															  + isnull(rc_ivarni,0) 
							end
														 as [Iva],
			...
			from 
				...
				RemitoCompra rc inner join Documento doc on rc.doc_id = doc.doc_id and @@conRemito <> 0
			                  
			where 
			
							  (			(rc_fecha <  @@Fini and @@nTipo = 0) 
									or  (rc_fecha <= @@Fini and @@nTipo in (1,2)) 
									or 	(			rc_fecha >= @@Fini
												and	rc_fecha <= @@Ffin
												and @@nTipo = 3
											)
...
--/////////////////////////////////////////////////////////////////////////
	...
	--/////////////////////////////////////////////////////////////////////////
		...
		where 
					...
					and (abs(round(fc_pendiente,2))>=@@saldominimo or @@soloDeudores = 0)
		...
		select 
		        ...
		        1                  as nOrden_id,
						...
						case rc.doct_id 
							when 25 then -(	rc_importedesc1 
														+ rc_importedesc2  
														)
							else						rc_importedesc1 
														+ rc_importedesc2
						end				 as [Descuento],
		...
		from 
			...
			RemitoCompra rc  inner join Proveedor prov 												on rc.prov_id 	= prov.prov_id
		                   ...
		                   left  join Estado est                            on rc.est_id    = est.est_id
		                   left  join Empresa emp                           on doc.emp_id   = emp.emp_id 
		                   left  join Sucursal suc                          on rc.suc_id    = suc.suc_id
		                   left  join Legajo lgj                            on rc.lgj_id    = lgj.lgj_id
		                   left  join CondicionPago cpg                     on rc.cpg_id    = cpg.cpg_id
		...
		where 
					...
					and (abs(round(rc_pendiente,2))>=@@saldominimo or @@soloDeudores = 0)
		...
		select 
		        ...
		        1                  as nOrden_id,
		 			  ...
		 			  0                  as [Iva],
						...
						''                 as [Moneda],
						est_nombre         as [Estado],
						isnull((select min(cue_nombre) 
						 from OrdenPagoItem opgi inner join cuenta cue on opgi.cue_id = cue.cue_id
						 where opg_id = opg.opg_id and opgi_tipo = 5),'Sin cuenta')
							                 as [Cuenta],
		        doc_nombre         as [Documento],
		        emp_nombre         as Empresa, 
		        suc_nombre         as [Sucursal],
		...
		where 
					...
					and (abs(round(opg_pendiente,2))>=@@saldominimo or @@soloDeudores = 0)
