/*---------------------------------------------------------------------
Nombre: Imputacion Contable de Documentos de Compra (Agrupa por cue_nombre, cue_codigo, [de compras], tercero, Fecha, [Comp. Origen])
---------------------------------------------------------------------*/
/*  
...
exec DC_CSC_COM_0210 1,'20060501','20070430','0','0','0',0,'1',0

*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_COM_0210]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_COM_0210]

go
create procedure DC_CSC_COM_0210 (
...
begin
	...
	create table #t_dc_csc_com_0210_as (as_id int)

	if @@bSinAsEj <> 0 begin
		insert into #t_dc_csc_com_0210_as(as_id)
	...
	create table #t_dc_csc_com_0210 (cue_id int)

	insert into #t_dc_csc_com_0210 (cue_id)
	...
	where fc_fecha between @@Fini and @@Ffin
		and est_id <> 7
		and not exists(select * from #t_dc_csc_com_0210 
									where cue_id = isnull(isnull(cue.cue_id,
																								provcueg.cue_id),
																				 cueg.cue_id)
								)

--	select cue_nombre,cue.cue_id from #t_dc_csc_com_0210 t inner join cuenta cue on t.cue_id = cue.cue_id
...
where 
			...
			and exists(select * from #t_dc_csc_com_0210 where cue_id = cue.cue_id)

			and not exists(select * from #t_dc_csc_com_0210_as where as_id = ast.as_id)
