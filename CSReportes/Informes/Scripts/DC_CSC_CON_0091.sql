...
---------------------------------------------------------------------*/
if exists (select * from sysobjects where id = object_id(N'[dbo].[DC_CSC_CON_0091]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[DC_CSC_CON_0091]


/*
select * from documento where doc_nombre like '%cob%'

 [DC_CSC_CON_0091] 1,'20060901 00:00:00','20060930 00:00:00','287','0','0',0,'0','0',0

*/

go
create procedure DC_CSC_CON_0091(
...
and   (isnull(ast.doc_id_cliente,0) = @doc_id or (ast.doc_id = @doc_id and ast.doc_id_cliente is null) or @doc_id=0)
...
and   (      (ast.doct_id = @doct_id and ast.doct_id_cliente is null)
        or  isnull(ast.doct_id_cliente,0) = @doct_id
        or   @doct_id  =0
      )
...
and   (
          (exists(select rptarb_hojaid 
                  ...
                  and  tbl_id = 4001
                  and  (rptarb_hojaid = isnull(ast.doc_id_cliente,0) or (rptarb_hojaid = ast.doc_id and ast.doc_id_cliente is null))
                 ) 
           )
        or 
           (@ram_id_documento = 0)
...
and   (
          (exists(select rptarb_hojaid 
                  ...
                  where
                       rptarb_cliente = @clienteID
                  and  tbl_id = 4003
                  and  (    (rptarb_hojaid = ast.doct_id and ast.doct_id_cliente is null)
                        or  rptarb_hojaid = isnull(ast.doct_id_cliente,0)
                        )
                 ) 
           )
        or 
