...
Public Function ZDLLPass(ByRef p As ZipCBChar, _
  ...
  ' Ask for password:
  m_cZip.PasswordRequest szpassword, N, m_bPasswordConfirm, bCancel
...
Public Function VBZip( _
   ...
   For i = 1 To iFileCount
      zZipFileNames.zFiles(i - 1) = sFileSpecs(i)
   Next i
   zZipFileNames.zFiles(i) = vbNullChar
  ...
  zEncrypt = tZPOPT.fEncrypt
  zComment = tZPOPT.fComment
  zJunkDir = tZPOPT.fJunkDir
  zRecurse = tZPOPT.fRecurse
  ...
  '-- Go Zip It Them Up!
  retcode = ZpArchive(iFileCount, sZipFile, zZipFileNames)
