...
Public Function GetMACAddress(ByVal idx As Long) As String
  Dim cnt             As Long
  ...
  If numLanas > 0 Then
    ...
    For cnt = 0 To numLanas - 1
      If idx = cnt Then
        If LenB(sMACAddresses(cnt)) Then
          address = sMACAddresses(cnt)
          Exit For
        End If
        If cnt > idx Then
          Exit Function
...
Private Function GetNBMacAddresses(sMACAddresses() As String) As Long
  ...
  'NCB structure
  If ncb.ncb_retcode = NRC_GOODRET And lana.length > 0 Then
